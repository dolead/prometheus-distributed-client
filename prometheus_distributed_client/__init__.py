...
class RedisValueClass(MutexValue):
    def __init__(
        self, typ, metric_name, name, labelnames, labelvalues, **kwargs
    ):
        super().__init__(
            typ, metric_name, name, labelnames, labelvalues, **kwargs
        )
        self.__name = name
...
class Counter(prometheus_client.Counter):
    ...
    def _multi_samples(self):
        for suffix in "_total", "_created":
            for labels, value in (
                get_redis_conn().hgetall(self._name + suffix).items()
            ):
                yield (
                    suffix,
